/**
 * Scorecard V4 Stock Analysis — Score vs Price Prediction Report
 *
 * Scores all BSE-listed non-banking stocks using V4 Non-Banking Scorecard
 * as of a start date, tracks price performance through an end date, and
 * analyzes whether higher scores predict higher price gains.
 *
 * Outputs:
 ...
 *
 * Usage:
 *   npx tsx --tsconfig tsconfig.json scripts/scorecard-v4-stock-analysis.ts
 *   npx tsx --tsconfig tsconfig.json scripts/scorecard-v4-stock-analysis.ts --limit 50
 */
...
async function main() {
  console.log('\n' + '='.repeat(70))
  console.log('  SCORECARD V4 — STOCK ANALYSIS REPORT')
  console.log('  Score vs Price Prediction Analysis')
  ...
  let report = `# Scorecard V4 — Stock Analysis Report\n`
  report += `## Score vs Price Prediction Analysis\n\n`
  report += `**Date Range:** ${START_DATE} to ${END_DATE} (~14 months)\n`
