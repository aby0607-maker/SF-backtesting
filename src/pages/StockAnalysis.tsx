        ...
        {showDemoSpotlights && spotlights.length > 0 && (
          <>
            ...
            <motion.div
              ...
              exit={{ opacity: 0, y: 20 }}
              className="fixed bottom-6 left-1/2 z-50 w-[360px] max-w-[calc(100vw-2rem)]"
            >
              <div className="bg-dark-800 rounded-2xl border border-primary-500/30 shadow-2xl shadow-primary-500/20 overflow-hidden">
                {/* Header */}
                <div className="p-4 bg-gradient-to-r from-primary-500/15 to-purple-500/15 border-b border-white/5">
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        <span className="text-[10px] text-primary-400 font-medium uppercase tracking-wider block">Feature Spotlight</span>
                        <span className="text-sm font-semibold text-white">{spotlights[activeSpotlightIndex].featureName}</span>
                      </div>
                    </div>
                    <button
                  ...
                  <div className="mt-3">
                    {(() => {
                      ...
                      return (
                        <span className={cn('inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold', info.bgColor, info.color)}>
                ...
                <div className="p-4 space-y-3">
                  {/* JTBD */}
                  <div>
                    <span className="text-[10px] font-semibold text-info-400 uppercase tracking-wider block mb-1">Job To Be Done</span>
                    <p className="text-sm text-neutral-200 leading-relaxed">{spotlights[activeSpotlightIndex].jtbd}</p>
                  </div>

                  {/* User Outcome */}
                  <div>
                    <span className="text-[10px] font-semibold text-success-400 uppercase tracking-wider block mb-1">You Get</span>
                    <p className="text-sm text-success-300 font-medium">{spotlights[activeSpotlightIndex].userOutcome}</p>
                  </div>

                  {/* Competitive Advantage */}
                  <div className="p-3 rounded-xl bg-warning-500/5 border border-warning-500/20">
                    <span className="text-[10px] font-semibold text-warning-400 uppercase tracking-wider block mb-1.5">vs Competition</span>
                    <p className="text-xs text-neutral-400 leading-relaxed">{spotlights[activeSpotlightIndex].competitiveAdvantage}</p>
                  </div>
                </div>
                ...
                <div className="px-4 py-3 bg-dark-700/50 border-t border-white/5 flex items-center justify-between">
                  <button
                    ...
                    className={cn(
                      ...
                      activeSpotlightIndex === 0 ? 'text-neutral-600 cursor-not-allowed' : 'text-neutral-400 hover:text-white'
                    )}
                  >
                  ...
                  <div className="flex items-center gap-1.5">
                    <span className="text-xs text-neutral-500">{activeSpotlightIndex + 1}</span>
                    <span className="text-neutral-600">/</span>
                    <span className="text-xs text-neutral-500">{spotlights.length}</span>
                  </div>
                  ...
                  <button
                    ...
                    className={cn(
                      'flex items-center gap-1 text-xs font-medium transition-colors',
                      activeSpotlightIndex === spotlights.length - 1 ? 'text-neutral-600 cursor-not-allowed' : 'text-neutral-400 hover:text-white'
                    )}
                  >
                    Next
                    <ChevronDown className="w-4 h-4 -rotate-90" />
